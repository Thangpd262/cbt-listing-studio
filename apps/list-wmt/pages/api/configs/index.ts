...
// PUT — upsert config (default shipping node + extra config JSON)
export default withAuth(async (req, res, auth) => {
  ...
  if (req.method === 'PUT') {
    const { default_shipping_node, config } = req.body ?? {}
    const { data, error: dbError } = await supabase
      .from('wmt_product_configs')
      .upsert(
        {
          ...
          ...(default_shipping_node !== undefined ? { default_shipping_node } : {}),
